...
<script type="text/javascript">
   ...
   /*창고 페이징 처리*/
   var pageSizeWarehouse = 12;
   var pageBlockSizeWarehouse = 12;
   ...
   function init() {
       Vue.component('my-component', { // 이렇게 선언하면 어디서든 사용가능
           template: '<div>전역 컴포넌트</div>'
       });

       warehouselist = new Vue({
           el: '#warehouseList', // div id 안에서만 사용 가능하도록 설정합니다.
           data: {
              
               listitem: [], // 기존 목록 데이터
               detailList: [], // 상세 목록 데이터
               sname : '',
               oname : 'all',
          param: {
           ...
           created: function () {
               axios.post('listWarehouseVue.do', this.param)
                   .then(response => {
                       this.listitem = response.data.listWarehouseModel;
           ...
           methods: {
               ...
               callfListProduct: function (warehouse_nm, warehouse_cd) {
                   axios.post('listWarehouseProductVue.do', {
                       ...
                       .then(response => {
                           this.detailList = response.data.listWarehouseProductModel;
                           console.log(response);
                       })
                       .catch(error => {
                           console.log(error);
               ...
               search: function () {
                  ...
                  console.log(this.oname);
                   axios.post('listWarehouseVue.do', {
               ...
               openWarehouseModal: function (warehouse_cd) {
            	   alert("창고모달!");
                  console.log(warehouse_cd);
                  ...
                  fPopModalWarehouse(warehouse_cd);
                   axios.post('saveWarehouse.do', {
                           warehouse_cd: warehouse_cd
                       })
                       .then(response => {
                    	   alert("수정성공!",response);
                           this.detailList = response.data.resultMap;
                           console.log(response);
                       })
                       .catch(error => {
                    	   alert("수정실패!",error);
   ...
   /* warehousemodal = new Vue({
           el: '#layer1', // div id 안에서만 사용 가능하도록 설정합니다.
      ...
      cancel: function () {
             console.log('cancel 메서드가 호출되었습니다.');
            gfCloseModal();
   ...
   function fListWarehouse(currentPage) {
      ...
      var oname = searchKey.options[searchKey.selectedIndex];
      ...
      callAjax("/scm/listWarehouseVue.do", "json", "text", true, param, resultCallback);
   }
   ...
   function flistWarehouseResult(data, currentPage) {
      ...
      //페이지 네비게이션 생성
      var paginationHtml = getPaginationHtml(currentPage, totalWarehouse,
            pageSizeWarehouse, pageBlockSizeWarehouse, 'fListWarehouse');
   ...
   function fValidateWarehouse() {
      var chk = checkNotEmpty([ [ "warehouse_cd", "창고코드를 입력하세요." ],
            [ "warehouse_nm", "창고명 입력하세요." ],
            [ "wh_mng_id", "담당자ID를 입력하세요." ], [ "zip_cd", "우편주소를 입력하세요." ],
            [ "addr", "주소를 입력하세요." ], [ "addr_detail", "상세주소를 입력하세요." ] ]);
      if (!chk) {
         return;
      }
      return true;
   ...
   function board_search(currentPage) {
      ...
      //callAjax("/scm/listWarehouseVue.do", "post", "text", true, param, resultCallback);
      callAjax("/scm/listWarehouseVue.do", "json", "text", true, param, resultCallback);
   }
...
<body>
   ...
   <form id="myForm" action="" method="">
      ...
      <input type="hidden" id="currentPageWarehouse" value="1"> <input
         type="hidden" id="currentPageProduct" value="1"> <input
         type="hidden" id="tmpwarehouse_nm" value=""> <input
         type="hidden" id="tmpwarehouse_cd" value=""> <input
                  ...
                  <div class="content">
                     <p class="Location">
                        <a href="/system/notice.do" class="btn_set home">메인으로</a> <a
                           class="btn_nav">기준 정보</a> <span class="btn_nav bold">창고정보
                           관리</span> <a href="" class="btn_set refresh">새로고침</a>
                     </p>
                     <p class="conTitle">
                        <span>창고정보</span> <span class="fr"> <a
                           href="javascript:fPopModalWarehouse()" class="btnType blue"
                     ...
                     <div class="WarehouseList" id="warehouseList">
                        <div class="conTitle"
                           ...
                           </select> <input type="text" style="width: 300px; height: 30px;"
                              v-model="sname"> <a href="#" @click="search"
                        ...
                        <table class="col">
                           ...
                           <tbody>
                              ...
                              <tr v-for="(item, index) in listitem" :key="index">
                                 ...
                                 <td><a href="#"
                                    @click="callfListProduct(item.warehouse_nm, item.warehouse_cd)">{{
                                       item.warehouse_nm }}</a></td>
                                 <td>{{ item.wh_mng_nm }}</td>
                                 <td>{{ item.tel }}</td>
                                 <td>{{ item.email }}</td>
                                 <td>{{ item.zip_cd }}</td>
                                 <td>{{ item.addr }}</td>
                                 <td><a href="#" class="btnType3 color1"
                                    @click="openWarehouseModal(item.warehouse_cd)"> <span>수정</span></a></td>
                              </tr>
                           ...
                           <table class="col">
                              ...
                              <tbody>
                                 ...
                                 <tr v-for="(detail, index) in detailList" :key="index">
                                    <td>{{ detail.product_cd }}</td>
                                    <td>{{ detail.prod_nm }}</td>
                                    <td>{{ detail.l_ct_nm }}</td>
                                    <td>{{ detail.supply_nm }}</td>
                                    <td>{{ detail.stock }}</td>
                                 </tr>
                        ...
                        <input type="hidden" id="totalWarehouse" name="totalWarehouse"
         ...
         <dl>
            ...
            <dd class="content">
                  ...
                  <tbody>
                     <tr>
                        ...
                        <td><input type="text" class="inputTxt p100"
                           id="warehouse_cd" v-model="warehouse_cd" maxlength="20" /></td>
                        <th scope="row">창고명 <span class="font_red">*</span></th>
                        <td><input type="text" class="inputTxt p100"
                           id="warehouse_nm" v-model="warehouse_nm" maxlength="50" /></td>
                     </tr>
                     <tr>
                        <th scope="row">담당자ID<span class="font_red">*</span></th>
                        <td><input type="text" class="inputTxt p100" id="wh_mng_id"
                           v-model="wh_mng_id" maxlength="50" /></td>
                        <th scope="row">담당자명</th>
                        <td><select id="wh_mng_nm" v-model="wh_mng_nm"
                           onChange="javascript:selectWarehouseMng()"></select></td>
                     </tr>
                     <tr>
                        <th scope="row">우편번호 <span class="font_red">*</span></th>
                        <td colspan="2"><input type="text" class="inputTxt p100"
                           id="zip_cd" v-model="zip_cd" maxlength="6" /></td>
                     ...
                     <tr>
                        ...
                        <td colspan="3"><input type="text" class="inputTxt p100"
                           id="addr" v-model="addr" maxlength="200" /></td>
                     </tr>
                     <tr>
                        <th scope="row">상세주소 <span class="font_red">*</span></th>
                        <td colspan="3"><input type="text" class="inputTxt p100"
                           id="addr_detail" v-model="addr_detail" maxlength="200" /></td>
                     </tr>
               ...
               <div class="btn_areaC mt30">
                  <a href="#" class="btnType blue" id="btnSaveWarehouse" name="btn"
                     @click="save"><span>저장</span></a> 
                  <a href="#" class="btnType blue" id="btnDeleteWarehouse" name="btn"
                     @click="deleteItem"><span>삭제</span></a> 
                  <a href="#" class="btnType gray" id="btnCloseWarehouse" name="btn"
                     @click="cancel"><span>취소</span></a>
               </div>
            </dd>
         </dl>
